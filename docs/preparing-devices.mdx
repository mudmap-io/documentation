...
- Enter a port number in **SSH Port** if you want SSH to use a non-default 
  ...
  :::
- It's now recommended that Mudmap's static IP's be granted login protection to
  prevent `sshguard` from blacklisting the server. This can cause installation
  issues. The list of IP addresses are found [below](#creating-a-rule).
- Click **Save**
...
<div style={{textAlign: 'center'}}>
  <img  alt="Preparing your pfSense firewall - Login Protection" src={useBaseUrl
  ('img/login-protection.png')} />
</div>
